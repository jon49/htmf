...
let hfTarget = "hf-target", hfSwap = "hf-swap", hf = "hf"
...
doc.addEventListener("submit", async e => {
    const active = doc.activeElement,
          ...
          submitters = [submitter, form].filter(x => x)

    if (submitters.find(hasAttr("hf-ignore"))
        || !([hfTarget, hf, hfSwap].find(x => submitters.find(hasAttr(x))))) {
        return
    }
    e.preventDefault()
